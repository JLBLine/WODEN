...
> Although ``WODEN`` is still very much a great tool for MWA interferomteric simulations, I (Jack Line) am no longer working in astronomy. I'll drop in to advise and/or fix bugs from time to time, but I can't commit to developing new features I'm afraid. If you want new features (or even better, want to write new features), please drop an issue on this GitHub page. Hopefully someone will have a look at i (maybe even Jack). If you end up taking over this project, feel free to delete this message!

> New in version 2.7-alpha:
 - Can now use versions >= 0.7.2 of `EveryBeam`, rather than relying on Jack's bespoke EveryBeam branch.
 - As a result however, to use these versions of `EveryBeam`, you have to use `casacore >= 3.7.0`, which means manual installation of `casacore` (`sudo apt install casacore-dev` is too old a version)
 - Installation documentation has been updated to reflect this
 - Note there are breaking changes in `EveryBeam`, so older version cannot be used. This has prompted breaking changes in `WODEN` in regards to `EveryBeam`, including the `run_everybeam` Python wrapper. See updated documentation for new arguments.
