...
do
    docker build --progress=plain --build-arg="CUDA_ARCH=${arch}" -t jlbline/woden-2.2:cuda-$arch -f Dockerfile_cuda .
    docker push jlbline/woden-2.2:cuda-$arch
done
...
docker build -t jlbline/woden-2.2:setonix -f Dockerfile_setonix .
