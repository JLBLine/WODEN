...
class Test(unittest.TestCase):
    ...
    def do_test(self):
        """First of all, run `run_everybeam` at full accuracy. Then run with the
        `full_accuaracy` flag set to False, and compare the results. """
        ...
        all_times = np.array([observing_time + TimeDelta(i, format='sec') for i in range(0, 10, 2)])
        ...
        j2000_latitudes = np.full(len(all_times), arr_latitude)
        j2000_lsts = np.full(len(all_times), np.radians(LST_deg))
        
        
        all_freqs = np.arange(100e+6, 101e+6, 10e+3)
        station_ids = np.array([0,10])
        
        slow_jones =  run_everybeam(ras, decs, beam_ra0, beam_dec0,
        ...
        fast_jones =  run_everybeam(ras, decs, beam_ra0, beam_dec0,
                                j2000_latitudes, j2000_lsts,
                                arr_latitude, arr_long,
                                all_times, all_freqs,
                                telescope,
        ...
        npt.assert_allclose(slow_jones, fast_jones, atol=6e-2, rtol=6e-2)
            
    def test_fast_everybeam(self):
