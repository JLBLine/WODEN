...
if __name__ == "__main__":
    ...
    wod_xx_default, wod_yy_default = read_uvfits("woden_default_band01.uvfits")
    wod_xx_deformed, wod_yy_deformed = read_uvfits("woden_dipamps_band01.uvfits")
    ...
    np.testing.assert_allclose(np.imag(wod_yy_deformed), np.imag(hyp_yy_deformed), atol=atol, rtol=rtol)
    ...
    if type(bins) == np.ndarray:
        ...
        axs[1,0].hist(np.abs(wod_xx_deformed), histtype='step',
                    label='E-W WODEN deformed', lw=2, hatch='/')
        axs[1,0].hist(np.abs(hyp_xx_deformed), histtype='step',
                    label='E-W hyperdrive deformed', lw=2)
        
        axs[1,1].hist(np.abs(wod_yy_deformed), histtype='step',
                    label='N-S WODEN deformed', lw=2, hatch='/')
        axs[1,1].hist(np.abs(hyp_yy_deformed), histtype='step',
                    label='N-S hyperdrive deformed', lw=2)
    ...
    fig.savefig('compare_woden_hyp_dipamps.png', bbox_inches='tight')
    plt.close()
