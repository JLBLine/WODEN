...
if __name__ == "__main__":
    ...
    wod_xx_default, wod_yy_default = read_uvfits("woden_hypbeam_noflags_band01.uvfits")
    wod_xx_deformed, wod_yy_deformed = read_uvfits("woden_hypbeam_dipflags_band01.uvfits")
    ...
    axs[0,0].hist(np.abs(hyp_xx_default), histtype='step',
                  label='E-W hyperdrive perfect', lw=2, hatch='\\')#, bins=bins)
    ...
    axs[0,1].hist(np.abs(hyp_yy_default), histtype='step',
                  label='N-S hyperdrive perfect', lw=2, hatch='\\')#, bins=bins)
    ...
    if type(bins) == np.ndarray:
        ...
        axs[1,0].hist(np.abs(wod_xx_deformed), histtype='step',
                    label='E-W WODEN-hyperbeam deformed', lw=2, bins=bins, hatch='/')
        axs[1,0].hist(np.abs(hyp_xx_deformed), histtype='step',
                    label='E-W hyperdrive deformed', lw=2, bins=bins, hatch='\\')
        axs[1,1].hist(np.abs(wod_yy_deformed), histtype='step',
                    label='N-S WODEN-hyperbeam deformed', lw=2, bins=bins, hatch='/')
        axs[1,1].hist(np.abs(hyp_yy_deformed), histtype='step',
                    label='N-S hyperdrive deformed', lw=2, bins=bins, hatch='\\')
        
    else:
    
        axs[1,0].hist(np.abs(wod_xx_deformed), histtype='step',
                    label='E-W WODEN-hyperbeam flagged', lw=2, hatch='/')
        axs[1,0].hist(np.abs(hyp_xx_deformed), histtype='step',
                    label='E-W hyperdrive flagged', lw=2, hatch='\\')
        
        axs[1,1].hist(np.abs(wod_yy_deformed), histtype='step',
                    label='N-S WODEN-hyperbeam flagged', lw=2, hatch='/')
        axs[1,1].hist(np.abs(hyp_yy_deformed), histtype='step',
    ...
    fig.savefig('compare_woden_hypbeam_dipflags.png', bbox_inches='tight')
    plt.close()
