...
if __name__ == "__main__":
    ...
    wod_xx_default, wod_yy_default = read_uvfits("woden_uvbeam_default_band01.uvfits")
    wod_xx_deformed, wod_yy_deformed = read_uvfits("woden_uvbeam_dipamps_band01.uvfits")
    ...
    axs[0,0].hist(np.abs(wod_xx_default), histtype='step', 
                  label='E-W WODEN-uvbeam amps=1', lw=2, hatch='/')#, bins=bins)
    ...
    axs[0,1].hist(np.abs(wod_yy_default), histtype='step',
                  label='N-S WODEN-uvbeam amps=1', lw=2, hatch='/')#, bins=bins)
    ...
    if type(bins) == np.ndarray:
        ...
        axs[1,0].hist(np.abs(wod_xx_deformed), histtype='step',
                    label='E-W WODEN-uvbeam bespoke amps', lw=2, bins=bins, hatch='/')
        ...
        axs[1,1].hist(np.abs(wod_yy_deformed), histtype='step',
                    label='N-S WODEN-uvbeam bespoke amps', lw=2, bins=bins, hatch='/')
        ...
        axs[1,0].hist(np.abs(wod_xx_deformed), histtype='step',
                    label='E-W WODEN-uvbeam bespoke amps', lw=2, hatch='/')
        axs[1,0].hist(np.abs(hyp_xx_deformed), histtype='step',
                    label='E-W hyperdrive bespoke amps', lw=2, hatch='\\')
        
        axs[1,1].hist(np.abs(wod_yy_deformed), histtype='step',
                    label='N-S WODEN-uvbeam bespoke amps', lw=2, hatch='/')
        axs[1,1].hist(np.abs(hyp_yy_deformed), histtype='step',
    ...
    plt.tight_layout()
    fig.savefig('compare_woden_uvbeam_dipamps.png', bbox_inches='tight')
    plt.close()
